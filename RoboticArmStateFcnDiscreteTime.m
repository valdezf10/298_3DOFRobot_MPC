function xk1 = RoboticArmStateFcnDiscreteTime(xk, uk, Ts)
% Nonlinear discrete-time state transition for the robotic arm model.
%
...
% RoboticArmStateFcn is the continuous-time state function for this
% example. Obtain the state derivatives at the current point.
xk = xk(:);     % Make sure inputs are column vectors
uk = uk(:);
ffun = @(xk,uk) RoboticArmStateFcn(xk,uk);
fk = ffun(xk,uk);
